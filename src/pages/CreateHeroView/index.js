...
export default class CreateHeroView extends Component {
  ...
  componentWillMount() {
    ...
    if(''.includes(this.state.hero.heroName)) {
      this.setState({
        disableButtonCreate: true,
        currentButtonColor: this.state.disableColor
      })
    } else {
      this.setState({
        disableButtonCreate: false,
        currentButtonColor: this.state.enableColor
      })
    }
  }
  ...
  createHero = () => {
    ...
    Alert.alert('Message', createHeroMsg.message)

    if(createHeroMsg.result) {
