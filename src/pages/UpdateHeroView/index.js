...
export default class UpdateHeroView extends Component {
  constructor(props) {
    super(props);
    let hero, event;
  ...
  changeName = (name) => {
    ...
    if (''.includes(this.state.hero.heroName)){
      ...
      this.setState({ hero, disableButtonCreate: false, currentButtonColor: this.state.enableColor });
    }
  }
  ...
  updateHero = () => {
    ...
    let updateHeroMsg = updateHero(this.state.hero);
    Alert.alert('Message', updateHeroMsg.message);

    if (updateHeroMsg.result) {
        if (this.state.event)
